...
/* FILENAME: 2Dvpt_inject.c */
/* VERSION: 3 (2011 Jun 04 - ...)
            With a resume option. (well, working on implementing it...) */
/* AUTHOR: Andrew Forrester <aforrester@ucla.edu> */
/* DESCRIPTION:
   * This C program calculates time-dependent properties of a 2D system of superfluid (a thin layer of liquid 4He) given either 0) initial conditions of equilibrium followed by injection of vortex pairs of a certain pair separation ai or 1) initial conditions of steady state with injection followed by shutting off the injection and a return to equilibrium.
...
/* EXT FILES: none */
/* COMPILE NOTES:
   * To compile, type "g++ -lm 2Dvpt_inject.c" without the quotes; then to run, type "./a.out".
...
main(){
	...
	asprintf(&filename1, "2Dvpt_inject_vsl_IC%i_T%g_lmax%g_dl%g_tmax%g_dt0%g_linj%g_InjConst%g_newBC.dat", IC, iTfrac, lmax, dl, tmax, dt0, linj, InjConst);
	asprintf(&filename2, "2Dvpt_inject_vst_IC%i_T%g_lmax%g_dl%g_tmax%g_dt0%g_linj%g_InjConst%g_newBC.dat", IC, iTfrac, lmax, dl, tmax, dt0, linj, InjConst);
	asprintf(&filename3a,"2Dvpt_inject_resumeA_IC%i_T%g_lmax%g_dl%g_tmax%g_dt0%g_linj%g_InjConst%g.dat", IC, iTfrac, lmax, dl, tmax, dt0, linj, InjConst);
	asprintf(&filename3b,"2Dvpt_inject_resumeB_IC%i_T%g_lmax%g_dl%g_tmax%g_dt0%g_linj%g_InjConst%g.dat", IC, iTfrac, lmax, dl, tmax, dt0, linj, InjConst);
	outfile1  = fopen(filename1,"w");    // E.g., "2Dvpt_inject.out"
	outfile2  = fopen(filename2,"w");    // E.g., "2Dvpt_inject.out"
	outfile3a = fopen(filename3a,"rw");  // E.g., "2Dvpt_inject_resumeA.out"
	outfile3b = fopen(filename3b,"rw");  // E.g., "2Dvpt_inject_resumeB.out"
	fprintf(outfile1,"# Filename: %s\n", filename1);
	fprintf(outfile2,"# Filename: %s\n", filename2);
	fprintf(outfile1,"# Source: 2Dvpt_inject.c\n");
	fprintf(outfile2,"# Source: 2Dvpt_inject.c\n");
