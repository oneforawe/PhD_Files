/* filename: calcDelC.c */
/* description: calculate Del/kB and C
 */
/* ext files: none */
...
main(){
	...
	outfile = fopen("ring3.out","w");  //  "ring3.out"
	...
	for(i=0; i<60; i++){
		...
		T = estimated.tempv*Tc;
		rho = r0+r1*T+r2*T*T+r3*T*T*T+r4*T*T*T*T;
		rhos = rs0+rs1*T+rs2*T*T+rs3*T*T*T+rs4*T*T*T*T;
		x = -(D1+D2*rho)/T;
		Delok = D1 + D2*rho + D3*exp(x)*T/rho + D4*exp(2*x)*rho + (D5+D6*rho+D7*rho*rho)*exp(3*x);

f(t,r) = a+b*r+c*exp(x)*t/r+d*exp(2*x)*r+(e+f*r+g*r**2)*exp(3*x)
x = -(a+b*r)/t
f(t,r) = a+b*r+c*exp(-(a+b*r)/t)*t/r+d*exp(-2*(a+b*r)/t)*r+(e+f*r+g*r**2)*exp(-3*(a+b*r)/t)



f(x,y) = a+b*y+c*exp(-(a+b*y)/x)*x/y+d*exp(-2*(a+b*y)/x)*y+(e+f*y+g*y**2)*exp(-3*(a+b*y)/x)

fit f(x,y) 'DelokFit.dat' using 1:2:3 via a,b,c,d,e,f,g





		C = m*m*kB*Delok/(PISQ*hbar*hbar*a0v*rhos);
