/* filename: RhosAmps.c */
/* description: This program calculates A'
     It should read in the data seen in plot_ring3_A_rhosAmps.png and calculate averages to be put into RhosAmps.dat and used for fitting K0c(P) and Cc(P)
 */
/* ext files: none */
...
haven't written the program
just eye-balled the values by looking at plot_ring3_A_rhosAmps.png
...
main(){
	...
	for(i=0; i<60; i++){
		...
		/* (Option C) Increment temperature value (near zero to just below Tc)
		...
		C = m*m*kB*Delok/(PISQ*hbar*hbar*a0v*rhos);  // (we set =1.3, value at 1.5K)
