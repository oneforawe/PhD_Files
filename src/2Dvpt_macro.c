...
/* FILENAME: 2Dvpt_macro.c */
/* VERSION: 3 (2012 Mar 10 - ...)
    This is the 2D version of ring1.c and was formerly named 2DKT.c.
    Now with the Helmholtz energy parameter e and the vortex-pair density vpdens.
    */
...
/* EXT FILES: none */
/* COMPILE NOTES:
   * To compile, type "g++ -lm 2Dvpt_macro.c" without the quotes; then to run, type "./a.out".
...
const double lmax     = 83.0;       //  1  //  maximum of the length scale l (potentially approximate)
const int    lsteps   = 83000;      //  1250, 5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
const double K0c      = 0.747853;  //  0.74785238  //  critical (T=Tkt) bare superfluid ratio (or "coupling constant")
// change K0c above.
// use latest value:   0.747852421772060461
// earlier value       0.747853
// earliest value      0.74785238
...
void EqRecRel(double x, double z[3], double dzdx[3], unsigned n);
...
int main(){
	...
	double x, l[lpts];
	double Tfrac, tempv;
	double z[3], K[lpts], G[lpts], e[lpts];
	double vpdens;
	double K0, G0, e0, y;
	FILE   *outfile;
	char   *filename;
	...
	if(TempLabelOp==0)  asprintf(&filename, "2Dvpt_macro_T_lmax%g_dl%g_Op%i.dat",  lmax,dl,Op);
	if(TempLabelOp==1)  asprintf(&filename, "2Dvpt_macro_Tv_lmax%g_dl%g_Op%i.dat", lmax,dl,Op);
	outfile = fopen(filename,"w");  //  E.g., "2Dvpt_constT.out"
	printf(     "\n\n# Filename: %s\n", filename);
	fprintf(outfile,"# Filename: %s\n", filename);
	fprintf(outfile,"# Source: 2Dvpt_macro.c\n");
	...
	fprintf(outfile,"%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","y","G","e","vpdens");
	...
	while(notdone==true){
		...
		z[1] = G[0] = G0 = exp(-4.0*l[0]-PISQ*K0)/a04;  // Villain model
		//z[1] = G[0] = G0 = exp(-4.0*l[0]-0.5*PISQ*K0)/a04;  // Villain model modified
		z[2] = e[0] = e0 = 0.0;
		...
		for(j=1; j<lpts; j++){
			...
			rk4(EqRecRel, &x, z, dl, 3);  // equil: rk4 3D (K,G,e), EqRecRel
			l[j] = l[j-1] + dl;
			K[j] = z[0];
			G[j] = z[1];
			e[j] = z[2];
			...
			//	printf(         "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", Tfrac, tempv, l[j], K[j], K[j]/K0, y, G[j], e[j], vpdens);
			//	fprintf(outfile,"%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", Tfrac, tempv, l[j], K[j], K[j]/K0, y, G[j], e[j], vpdens);
			//}
		}

		// Record data values //
		printf(         "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", Tfrac, tempv, l[lpts-1], K[lpts-1], K[lpts-1]/K0, y, G[lpts-1], e[lpts-1], vpdens);
		fprintf(outfile,"%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", Tfrac, tempv, l[lpts-1], K[lpts-1], K[lpts-1]/K0, y, G[lpts-1], e[lpts-1], vpdens);
		fflush(outfile);
	}
