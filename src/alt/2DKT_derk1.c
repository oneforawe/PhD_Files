...
double derk1(RETARRAY (*func)(double l, double z[]), STATE calculated, double Dl, unsigned n){
	double dl;
	int i,j,k;
	RETARRAY DthrmvDl[4];
	STATE test[3];
	...
	for(i=0; i<3; i++){
		...
		for(j=0; j<1; j++){
			...
			test[i].thrmv[1] = calculated.thrmv[1];
		}
		test[i].l = calculated.l + Dl*((i+1)/2)/2;  // See "Note" below.
		DthrmvDl[i+1] = func(IN test[i].l, IN test[i].thrmv);  //  PROBLEM HERE... with test[i].thrmv[1] numbers...
	}
	...
	for(j=0; j<1; j++) calculated.thrmv[j] += Dl*(DthrmvDl[0].arr[j]+2*(DthrmvDl[1].arr[j]+DthrmvDl[2].arr[j])+DthrmvDl[3].arr[j])/6;
	// calculated.l += Dl;

	/* Return the new calculated K-value (not whole state) */
	return calculated.thrmv[0];
}
