...
/* VERSION: 2 (2011 May 24 - 2011 Jun 04) */
...
const int    IC       = 0;            //  initial condition (0 -> equilibrium -> inject; 1 -> stead-state injection -> stop injection)
...
const double InjConst = 1.0e6;        //  injection constant - the factor multiplying the Heaviside step function, gamma*t0/(2*PI*a02), where gamma is the factor multiplying the dirac delta source
...
main(){
	...
	asprintf(&filename1, "2DKT_FPinject_vsl_IC%i_T%g_lmax%g_dl%g_tmax%g_dt0%g_linj%g_InjConst%g.dat", IC, iTfrac, lmax, dl, tmax, dt0, linj, InjConst);
	asprintf(&filename2, "2DKT_FPinject_vst_IC%i_T%g_lmax%g_dl%g_tmax%g_dt0%g_linj%g_InjConst%g.dat", IC, iTfrac, lmax, dl, tmax, dt0, linj, InjConst);
	...
	fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, linj=%g, InjConst=%g, tmax=%g, dt0=%g, a0=%g, a04=%g, K0c=%g\n", iTfrac,qTfrac,lmax,lsteps,dl,lpts,ldatamax,linj,InjConst,tmax,dt0,a0,a04,K0c);
	fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, linj=%g, InjConst=%g, tmax=%g, dt0=%g, a0=%g, a04=%g, K0c=%g\n", iTfrac,qTfrac,lmax,lsteps,dl,lpts,ldatamax,linj,InjConst,tmax,dt0,a0,a04,K0c);
	...
	////////////////////////////////////////////////
	//                                            //
	//  (n_t>=1) After Adding/Removing Injection  //
	//                                            //
	////////////////////////////////////////////////
	...
	//while(n_t<=5){
		...
		G[lpts-1] = 2.0*G[lpts-2] - G[lpts-3];
