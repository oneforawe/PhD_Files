...
/* DESCRIPTION:
   ...
   * DK0         - "pre-initial" adjustment of K0.  It's a "pre-initial" adjustment because we change it (divide by 2) before using it.
...
/* COMPILE NOTES:
   * To run, type "gcc -lm vlt_K0cFind.c vlt_derk.c" without the quotes.
...
main(){
	...
	infile  = fopen("vlt_CcK01Input3.dat","r");
	outfile = fopen("vlt_CcK0cOutput3.dat","w");
	fprintf(outfile,"%s\t%s\n","Cc","K0c");
	printf("%s\t%s\n","Cc","K0");
	...
	while(!feof(infile)){
		...
		/* Set K0 to initial guess K01 (which should be close to actual K0c) */
		K0 = K01;
