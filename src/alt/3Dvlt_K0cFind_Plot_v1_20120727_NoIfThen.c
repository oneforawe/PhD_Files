...
/* VERSION: 1 (2011 Aug 22 - 2012 Jul 27)
...
main(){
	...
	infile   = fopen("3Dvlt_K0cFind_Plot_Input4.dat","r");
	outfile1 = fopen("3Dvlt_K0cFind_Plot_Output4.dat","w");
	outfile2 = fopen("3Dvlt_K0cFind_Plot_OutputDetail.dat","w");
	...
	fprintf(outfile1,"# Parameter values: B=%21.21g, THETA=%g, N=%i, dl=%g, LDataPerOne=%i, MRecDataSets=%i, Kthresh=%g, uncertainty=%g, DK01=%g\n", B, THETA, N, dl, LDataPerOne, MRecDataSets, Kthresh, uncertainty, DK01);
	// fprintf(outfile,"%s\t%s\n","Cc","K0c");
	printf(         "%s\t%s\n","Cc","K0c");
	...
	while(!feof(infile)){
		// Increment Cc,K0c data set count (for output file) //
		i++;
		...
		asprintf(&filename, "3Dvlt_K0cFind_Plot_Cc%3.2f_K01%g.dat", Cc,K01);
		outfile3 = fopen(filename,"w");  //  E.g., "3Dvlt_K0cFind_Plot.out"
		...
		while(DK0>uncertainty){
			...
			while(1==1){
				...
				if( (j%DataSkip==0 || DK0/2<uncertainty) && k%LSkip==0)
					fprintf(outfile3,"%g\t%17.17g\t%17.17g\n", l, K, y);
		...
		// Close output file for certain Cc,K01 pair //
		fclose(outfile3);
	}
