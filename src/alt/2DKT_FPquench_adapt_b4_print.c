...
// before making it adaptive.  I saved this file because it outputs G(l) without having the glitchy drop from G[0] that shows up from the output of 2DKT_FPquench.c VERSION: 1 (2010 12 09).

/* FILENAME: 2DKT_FPquench_adapt.c */
/* VERSION: 1 (2010 12 19)
...
main(){
	...
	sprintf(filename, "2DKT_FPquench_adapt_lmax%i_dl%g_tmax%g_dt0%g.dat", lmax, dl, tmax, dt0);
