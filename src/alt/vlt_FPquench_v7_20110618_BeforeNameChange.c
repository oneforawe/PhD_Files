...
CHECK THIS CODE, SEE IF IT'S CORRECT:

k1[0] = k1[1];
g1[0] = g[0] +0.5*k1[0];
k1[maxsteps] = 2.0*k1[maxsteps-1]-k1[maxsteps-2];
g1[maxsteps] = g[maxsteps]+0.5*k1[maxsteps];

k2[0] = k2[1];
g2[0] = g[0]-k1[0]+k2[0];
k2[maxsteps] = 2.0*k2[maxsteps-1]-k2[maxsteps-2];
g2[maxsteps] = g[maxsteps]-k1[maxsteps]+k2[maxsteps];

g[0] = 2.0*g[1]-g[2];
g[maxsteps] = g[maxsteps-1]/(1+2*PI*k[maxsteps]*stepsize);



const double BB21=0.5,
	     BB31=-1.0,    BB32=1.0,
	      AA1=0.0,      AA2=0.0,      AA3=0.0,
	      CC1=1.0/6.0,  CC2=2.0/3.0,  CC3=1.0/6.0;


// Runge-Kutta 3th order method : Version 2 //
void rk3_2(void (*derivs)(double, double*, double*, unsigned), double *x, double h, double y[], unsigned n){
	int i;
	double *k[3],*ytest;

	for(i=0;i<3;i++)  k[i] = (double *)calloc(n,sizeof(double));  // the k's are slopes, dy/dx
	ytest = (double *)calloc(n,sizeof(double));

	derivs(*x+h*AA1,y,k[0],n);      // get k1=k[0]
	for(i=0;i<n;i++)  ytest[i] = y[i]+h*(0.5*k[0][i]);
	derivs(*x+h*AA2,ytest,k[1],n);  // get k2=k[1]
	for(i=0;i<n;i++)  ytest[i] = y[i]+h*(-k[0][i]+k[1][i]);
	derivs(*x+h*AA3,ytest,k[2],n);  // get k3=k[2]

	for(i=0;i<n;i++)  y[i] += h*( (1/6)*k[0][i] + (2/3)*k[1][i] + (1/6)*k[2][i] );
	*x += h;

	for(i=0;i<3;i++)  free((char *)k[i]);
	free((char *)ytest);
}
...
void rk3_2(void (*derivs)(double, double*, double*, unsigned), double *x, double h, double y[], unsigned n){
	...
	derivs(*x+h*AA1,y,k[0],n);      // get k1=k[0]
	for(i=0;i<n;i++)  ytest[i] = y[i]+h*(BB21*k[0][i]);
	derivs(*x+h*AA2,ytest,k[1],n);  // get k2=k[1]
	for(i=0;i<n;i++)  ytest[i] = y[i]+h*(BB31*k[0][i]+BB32*k[1][i]);
	derivs(*x+h*AA3,ytest,k[2],n);  // get k3=k[2]

	for(i=0;i<n;i++)  y[i] += h*(CC1*k[0][i]+CC2*k[1][i]+CC3*k[2][i]);
	*x += h;
...
/* VERSION: 7 (2011 Apr 29 - 2011 Jun 18)
...
const int    lsteps   = 2500;      // 12500, 5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
const double fp3 = (THETA/(1.0-THETA))*(THETA/(1.0-THETA)-2.0);  // = -0.75  not  -3*THETA/(1-THETA) = -4.50
const double fp4 = 2.0*THETA/(1.0-THETA)-2.0;                    // =  1.00  not  -3+THETA/(1-THETA) = -1.50
/* pulling q out of the derivative: "pqo" => "4"
const double fp1 = THETA/(1.0-THETA)-3.0;  // = -1.50
const double fp2 = THETA/(1.0-THETA);      // =  1.50
const double fp3 = -3*THETA/(1-THETA);     // = -4.50
const double fp4 = THETA/(1-THETA)-3;      // = -1.50
*/
...
main(){
	...
	//	}
		...
		for(s2=1; s2<=5; s2++){
			switch(s2){
				case 1: iTfrac = 1.00; qTfrac = 0.5; break;
...
//			outfile3b = fopen(filename3b,"rw");  // E.g., "vlt_FPquench_resumeB.out"
			...
			printf(          "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1-iTfrac, l[0], K[0], K[0]/K0, Kr[0]/K0, G[0], dG[0]);
			fprintf(outfile1,"%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1-iTfrac, l[0], K[0], K[0]/K0, Kr[0]/K0, G[0], dG[0]);
			...
			while(i<lpts){
				...
				if(G[i]!=G[i]){ // if G goes to infinity (nan)
					i -= 2;
					fprintf(outfile1,"#newlpts = %i\n", newlpts);
					break;
				}
				...
				if(i%(lsteps/ldatamax)==0){
					printf(          "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1-iTfrac, l[i], K[i], K[i]/K0, Kr[i]/K0, G[i], dG[i]);
					fprintf(outfile1,"%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%i\n", iTfrac, 1-iTfrac, l[i], K[i], K[i]/K0, Kr[i]/K0, G[i], dG[i], newlpts);
				}
			}
			newlpts = i;  // for use if I reinsert Kthresh limits
			fprintf(outfile1,"#newlpts = %i\n", newlpts);
			l[newlpts] = l[newlpts-1] + dl;  // I don't need this, do I?
			...
			newlpts = i;
			fprintf(outfile1,"#newlpts = %i\n", newlpts);
			fprintf(outfile1,"\n# (Mid-step) time step n_t = %ld\tt = %e\t(dt = %e)\n", n_t,t,dt);
			fprintf(outfile1,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","Kr/K0","G","dG");
			for(i=0; i<newlpts; i++){
				// Print at most [ldatamax] (e.g., 1000) data points to screen and output file //
				if(i%(lsteps/ldatamax)==0){
					printf(          "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", qTfrac, 1-qTfrac, l[i], K[i], K[i]/K0, Kr[i]/K0, G[i], dG[i]);
					fprintf(outfile1,"%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", qTfrac, 1-qTfrac, l[i], K[i], K[i]/K0, Kr[i]/K0, G[i], dG[i]);
				}
			}
...
//				dt = dt0 + Dt*(1-exp(-t));  // added
				...
				k1[newlpts-1]     = 2.0*k1[newlpts-2] - k1[newlpts-3];  //*
				testG1[newlpts-1] = G[newlpts-1] + 0.5*k1[newlpts-1];   //*
				...
				k2[newlpts-1]     = 2.0*k2[newlpts-2] - k2[newlpts-3];             //*
				testG2[newlpts-1] = G[newlpts-1] - k1[newlpts-1] + k2[newlpts-1];  //*

				for(i=1; i<newlpts-1; i++){
					fpGRecRel(l[i], dl, testG2);
					k3[i] = dGdt[0]*dt;
					if(i==1){ fprintf(outfile1,"#first, i=%i\tG[i]=%g\t(G[i]!=G[i])=%i\tk1[i]=%g\tk2[i]=%g\tk1[i]=%g\n", i, G[i], (G[i]!=G[i]), k1[i], k2[i], k3[i] ); }
					G[i]  = G[i] + k1[i]/6.0 + 2.0*k2[i]/3.0 + k3[i]/6.0;
					if(G[i]!=G[i]){ // if G goes to infinity (nan)
						newlpts = i-2;
						fprintf(outfile1,"\n#then, i=%i\tG[i]=%g\t(G[i]!=G[i])=%i\n", i, G[i], (G[i]!=G[i]) );
						fprintf(outfile1,"#newlpts = %i\n", newlpts);
						break;
					}
					dG[i] = k1[i]/6.0 + 2.0*k2[i]/3.0 + k3[i]/6.0;
					loopintegral += G[i]*exp(3*l[i])*dl;
				}
				...
				if( THETA*log(K[newlpts-1]) > 0.0 ){ // if ac' > a, set ac' = a (set THETA*log(K[newlpts-1]) to zero)
					Gtemp         = G[newlpts-2]/(1.0+PISQ*K[newlpts-1]*dl);                                //*
					dG[newlpts-1] = Gtemp-G[newlpts-1];                                                     //*
					G[newlpts-1]  = Gtemp;                                                                  //*
				}                                                                                               //*
				else{                                                                                           //*
					Gtemp         = G[newlpts-2]/(1.0+PISQ*K[newlpts-1]*(1.0-THETA*log(K[newlpts-1]))*dl);  //*
					dG[newlpts-1] = Gtemp-G[newlpts-1];                                                     //*
					G[newlpts-1]  = Gtemp;                                                                  //*
				}                                                                                               //*
				// The line above is based on the equilibrium relation dG/dl = G(6-6l-pi^2K(1-THETA*ln K)).
				// Could instead use: G[lpts-1] = G[lpts-2] + G[lpts-2](6.0-6.0*l[lpts-2]-PISQ*K[lpts-2]*(1.0-THETA*log(K[lpts-2])))*dl;
				// Shouldn't the above be based on... something else (non-equil)?
				loopintegral += G[0]*exp(3*l[0])*dl;
				loopintegral += G[newlpts-1]*exp(3*l[newlpts-1])*dl;
				loopdens = 4*PI*a03*loopintegral;
//				// Boundary, dummy point       // If the program works without this, delete.
				Gtemp       = G[newlpts-1];
				dG[newlpts] = Gtemp-G[newlpts];
				G[newlpts]  = G[newlpts-1];

				// Calculate K at progressively larger length scales via Runge-Kutta method (rk K) //
				x = 0.0;
				Kcalc[0] = K[0];
				i = 0;
				while(i<newlpts){
				...
				// temporary check:
//				printf("\n\n\n \t\t newlpts = %i\n\n\n\n", newlpts);
				// Boundary, dummy point  (CAN-BE/IS INCLUDED IN WHILE LOOP ABOVE)
//				K[newlpts] = ;
				// The line above is based on the non-equilibrium relation dK/dl = K - #*exp(6l)*G*K^2.
...
void fpKRecRel(double x, double Kcalc[1], double dKdx[1], unsigned n){
	dKdx[0] = Kcalc[0]-B*a06*exp(6.0*x)*G[i-1]*Kcalc[0]*Kcalc[0];
}
...
void fpGRecRel(double x, double Dl, double Gcalc[lpts+1]){
	if( THETA*log(K[i]) > 0.0 ) // if ac' > a, set ac' = a (set THETA*log(K[i]) to zero)
		dGdt[0] = 4*FPconst*exp(fp1*x)*(  ( fp2*Gcalc[i]*K[i] + (K[i+1]*Gcalc[i+1]-K[i-1]*Gcalc[i-1])/(2*Dl) )  -  THETA*Gcalc[i]*(K[i+1]-K[i-1])/(2*Dl)  +  ( fp3*Gcalc[i] + fp4*(Gcalc[i+1]-Gcalc[i-1])/(2*Dl) + (Gcalc[i+1]-2*Gcalc[i]+Gcalc[i-1])/(Dl*Dl) )/PISQ  );
	else
		dGdt[0] = 4*FPconst*exp(fp1*x)*(  (1-THETA*log(K[i]))*( fp2*Gcalc[i]*K[i] + (K[i+1]*Gcalc[i+1]-K[i-1]*Gcalc[i-1])/(2*Dl) )  -  THETA*Gcalc[i]*(K[i+1]-K[i-1])/(2*Dl)  +  ( fp3*Gcalc[i] + fp4*(Gcalc[i+1]-Gcalc[i-1])/(2*Dl) + (Gcalc[i+1]-2*Gcalc[i]+Gcalc[i-1])/(Dl*Dl) )/PISQ  );
}
