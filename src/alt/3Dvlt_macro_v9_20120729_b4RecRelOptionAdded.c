...
/* VERSION: 9 (2011 Aug 17 - 2012 Jul 29)
...
int main(){
	...
	//for(s=1; s<=1; s++){
		...
		if(TempLabelOp==0)  asprintf(&filename, "3Dvlt_macro_noKth_T_Cc%3.2f_lmax%g_dl%g_Op%i_usingK0cFind1.dat",  Cc,lmax,dl,Op);
//		if(TempLabelOp==0)  asprintf(&filename, "3Dvlt_macro_noKth_T_Cc%3.2f_lmax%g_dl%g_Op%i_usingK0cFind3dl.dat",  Cc,lmax,dl,Op);
//		if(TempLabelOp==0)  asprintf(&filename, "3Dvlt_macro_noKth_T_Cc%3.2f_lmax%g_dl%g_Op%i_usingK0cFind3ds.dat",  Cc,lmax,dl,Op);

//		if(TempLabelOp==0)  asprintf(&filename, "3Dvlt_macro_nitKth_T_Cc%3.2f_lmax%g_dl%g_Op%i_usingK0cFind1.dat",  Cc,lmax,dl,Op);
//		if(TempLabelOp==0)  asprintf(&filename, "3Dvlt_macro_nitKth_T_Cc%3.2f_lmax%g_dl%g_Op%i_usingK0cFind3dl.dat",  Cc,lmax,dl,Op);
//		if(TempLabelOp==0)  asprintf(&filename, "3Dvlt_macro_nitKth_T_Cc%3.2f_lmax%g_dl%g_Op%i_usingK0cFind3ds.dat",  Cc,lmax,dl,Op);
...
/*						case 0:   Tfrac=1.00;  tempv=1.0-Tfrac;  i++;            break;
			...
			// Record data values //
			printf(         "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", Tfrac, tempv, l[lpts-1], K[lpts-1], y[lpts-1], e[lpts-1], K[lpts-1]/K0, Kr[lpts-1]/K0, G[lpts-1]);
			fprintf(outfile,"%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", Tfrac, tempv, l[lpts-1], K[lpts-1], y[lpts-1], e[lpts-1], K[lpts-1]/K0, Kr[lpts-1]/K0, G[lpts-1]);
			fflush(outfile);
		}
