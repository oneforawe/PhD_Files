...
const int    lsteps   = 125000;      //  5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
main(){
	...
	//	}
		...
		for(s2=1; s2<=5; s2++){
			...
			asprintf(&filename1, "vlt_FPquench_v5bb_vsl_3_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt0%g_equil.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt0);
			asprintf(&filename2, "vlt_FPquench_v5bb_vst_3_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt0%g_equil.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt0);
