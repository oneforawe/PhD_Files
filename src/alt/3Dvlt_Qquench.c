...
/* FILENAME: 3Dvlt_Qquench.c */
/* VERSION: 1 (2011 Jun 29 - ...)
            This plan failed... the equations with Q (not pulling q through the derivative) are too complicated.
            
            With dt determined by limiting relationship between dl and dt.
            With options for 3rd order Runge-Kutta method.
            With large-scale boundary conditions based on number-density-flow derivations.
            With multiple options for boundary conditions of Gamma.
            Using K,y recursion relations (since the K,G recursion relations produced a bad equilibrium Gamma distribution at Tc -- need more lsteps to make that technique work nicely)
            Not allowing ac' to exceed a (See *), not allowing Gamma to fall below its equilibrium curve.
...
/* EXT FILES: none */
/* COMPILE NOTES:
   * To run, type "g++ -lm 3Dvlt_Qquench.c" without the quotes.
...
const int    lsteps    = 10000;      // 12500, 5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
const double tmax      = 1.0;        //  10000.0 //  max unitless time (see if(t==...) below)
...
const double Qconst    = 0.0;        //  Diffusion-related parameter
...
const double fp3 = -3*THETA/(1.0-THETA);   // = -4.50
const double fp4 = THETA/(1.0-THETA)-3;    // = -1.50

// Constants for lineintegral and Q //
const double li  = 1.0/(1.0-THETA)+3.0;     // =  5.50
const double qc1 = 1.0/(1.0-THETA);         // =  2.50
const double qc2 = 1.0/(1.0-THETA)-6.0;     // = -3.50
...
main(){
	...
	double  Kr[lpts];
	double  z[2], y, Kcalc[lpts], Gcalc[lpts];
	...
	//	}
		...
		for(s2=1; s2<=5; s2++){
			switch(s2){
				case 1: iTfrac = 1.00; qTfrac = 0.7; break;
			...
			asprintf(&filename1, "3Dvlt_Qquench_vsl_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt%g_BC%i.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt, BC);
			asprintf(&filename2, "3Dvlt_Qquench_vst_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt%g_BC%i.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt, BC);
...
//			outfile3b = fopen(filename3b,"rw");  // E.g., "3Dvlt_quench_resumeB.out"
			...
			fprintf(outfile1,"# Source: 3Dvlt_Qquench.c\n");
			fprintf(outfile2,"# Source: 3Dvlt_Qquench.c\n");
			...
			fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, t0, tmax=%g, dt=%g, (dtfrac=%g), a0=%g, a02=%g, a03=%g, a04=%g, a06=%g, Lconst=%g, NDflowTOL=%g, fp1=%g, fp2=%g, fp3=%g, fp4=%g, li=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,t0,tmax,dt,dtfrac,a0,a02,a03,a04,a06,Lconst,NDflowTOL,fp1,fp2,fp3,fp4,li);
			fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, t0, tmax=%g, dt=%g, (dtfrac=%g), a0=%g, a02=%g, a03=%g, a04=%g, a06=%g, Lconst=%g, NDflowTOL=%g, fp1=%g, fp2=%g, fp3=%g, fp4=%g, li=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,t0,tmax,dt,dtfrac,a0,a02,a03,a04,a06,Lconst,NDflowTOL,fp1,fp2,fp3,fp4,li);
			...
			Q[0]  = exp(qc1*l[i])*(6.0*l[0]-log(y))/(6.0*l[0]-log(y)-1.0) + Qconst*exp(6.0*l[0])/(iTfracT*y);
			...
			fprintf(outfile1,"\n# time step n_t = %ld\tt = %e\t(dt = %e)\n", n_t,t,dt);
			printf(            "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","Kr/K0","G","Q");
			fprintf(outfile1,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","Kr/K0","G","Q");
			printf(          "%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1.0-iTfrac, l[0], K[0], K[0]/K0, Kr[0]/K0, G[0], Q[0]);
			fprintf(outfile1,"%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1.0-iTfrac, l[0], K[0], K[0]/K0, Kr[0]/K0, G[0], Q[0]);
			fprintf(outfile2,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "time-step","t","T/Tc","1-T/Tc","l","K","K/K0","Kr/K0","G","Q","loopdens","linedens");
			...
			while(i<lpts){
				...
				Kr[i] = K[i]*exp(-l[i]);
				qintegral    += exp(qc2*l[i])*(y*(6.0*l[i]-log(y))/(6.0*l[i]-log(y)-1.0))*()*dl;
				loopintegral += G[i]*exp(3.0*l[i])*dl;
				lineintegral += G[i]*exp(li*l[0])*dl;
				Q[i]  = exp(qc1*l[i])*(6.0*l[0]-log(y))/(6.0*l[0]-log(y)-1.0) + Qconst*exp(6.0*l[0])/(iTfracT*y);
			...
			linedens = 2.0*PI*a04*Lconst*lineintegral;
			fflush(outfile1);
			...
			//while(n_t<=5){
				...
				// G[lpts-1] = G[lpts-2] + G[lpts-2]*(3-PISQ*K[lpts-2]*(1.0-THETA*log(K[lpts-2])))*dl;
				// or G[lpts-1] = G[lpts-2]/( 1.0 - (3-PISQ*K[lpts-1]*(1.0-THETA*log(K[lpts-1])))*dl );
				...
				//	G[lpts-1] = G[lpts-2] + G[lpts-2]*(3-PISQ*K[lpts-2])*dl;
				//else    G[lpts-1] = G[lpts-2] + G[lpts-2]*(3-PISQ*K[lpts-2]*(1.0-THETA*log(K[lpts-2])))*dl;
				// With the following code, G stays positive:
				if( log(K[lpts-1]) > 0.0 ) // if ac' > a, set ac' = a (replace log(K[lpts-1]) with zero)
					G[lpts-1] = G[lpts-2]/( 1.0 - (3-PISQ*K[lpts-1])*dl );
				else    G[lpts-1] = G[lpts-2]/( 1.0 - (3-PISQ*K[lpts-1]*(1.0-THETA*log(K[lpts-1])))*dl );
				...
				switch(BC){
					...
					case 3: // My bndry cond. Total number-density outflow must occur at smallest scale.
						if( log(K[0]) > 0.0 ) // if ac' > a, set ac' = a (replace log(K[lpts-1]) with zero)
							G[0] = (PI*t0*(loopdens_old-loopdens)*dl/(16.0*Lconst*a03) - G[1]*dt) / ( (3-PISQ*K[0])*dl*dt + PISQ*t0*dl*dl/(8.0*Lconst) - dt);
						else    G[0] = (PI*t0*(loopdens_old-loopdens)*dl/(16.0*Lconst*a03) - G[1]*dt) / ( (3-PISQ*K[0]*(1.0-THETA*log(K[0])))*dl*dt + PISQ*t0*dl*dl/(8.0*Lconst) - dt);
						if(G[0]<=G0final)  G[0] = G0final;
						break;
				}
				loopintegral += 0.5*G[0]*exp(3.0*l[0])*dl;
...
void fpKRecRel(double x, double Kcalc[1], double dKdx[1], unsigned n){
	dKdx[0] = Kcalc[0]-0.5*B*a06*exp(3.0*x)*G[i-1]*Kcalc[0]*Kcalc[0];
}
