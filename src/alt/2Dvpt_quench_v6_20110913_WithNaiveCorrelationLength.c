...
/* VERSION: 6 (2011 Sep 02 - 2011 Sep 13)
...
const double tmax      = 100.0;        //  10000.0 //  max time (in units of the diffusion time)
...
int main(){
	...
	double  dlnrhodlnt, xi, zscale, rhopowthry;
	...
	w  = 0;  // correlation length integer
	xi = l[w];
	...
	fprintf(outfile2,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "time-step", "t", "T/Tc","1-T/Tc","l","K","K/K0","G","pairdens","d(lnrho)/d(lnt)","xi","rhopowthry");
	...
	zscale = 2.0*PI*Ki[w] - 2.0;
	rhopowthry = -zscale/2.0;

	// Print out the data at largest length scale to file 2 //
	fprintf(outfile2,"%ld\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t\t%g\t%g\n", n_t, t, iTfrac, 1.0-iTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, G[lpts-1], pairdens, xi, rhopowthry);
	fflush(outfile2);
	...
	//while(n_t<=5){
		...
		if(t>ttrig){
			fprintf(outfile2,"%ld\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", n_t, t, qTfrac, 1-qTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, G[lpts-1], pairdens, dlnrhodlnt, xi, rhopowthry);
			fflush(outfile2);
			ttrig += dt*pow(10.0,0.05*k);
			//ttrig += dt0*exp(0.05*k);
			k++;
			// Sparse update //
			printf(          "n_t=%ld  t=%e  (dt=%e)  l=%g  K/K0=%g  G=%g  pairdens=%g  (Tfrac: %g->%g)  xi=%g  (exp: calc=%g thry=%g)\n", n_t, t, dt, l[lpts-1], K[lpts-1]/K0, G[lpts-1], pairdens, iTfrac, qTfrac, xi, dlnrhodlnt, rhopowthry);
		}
