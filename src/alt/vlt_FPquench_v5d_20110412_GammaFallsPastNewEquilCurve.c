...
/* VERSION: 5 (2011 Apr 05 - 2011 Apr 12)
            I'm using this version to figure out why I'm getting discontinuities at the beginning of K vs l, in my version 7, as I fix it.
            From version 5c to 5d I'm adding the updated boundary equations in the 3rd order rk for Gamma (G).
...
const int    lsteps   = 12500;      //  5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
main(){
	...
	//	}
		...
		for(s2=1; s2<=1; s2++){
			switch(s2){
				case 1: iTfrac = 1; qTfrac = 0.50; break;
...
//				printf("\n\n\n \t\t newlpts = %i\n\n\n\n", newlpts);
				...
				if( n_t==1 || n_t==2 || n_t==3 || n_t==4 || n_t==5 ){  // 1==0    ){// 
					fprintf(outfile1,"\n# time step n_t = %ld\tt = %e\t(dt = %e)\n", n_t,t,dt);
					fprintf(outfile1,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","Kr/K0","G");
					for(i=0; i<newlpts; i++){
