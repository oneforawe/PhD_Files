...
/* VERSION: 8 (2011 Jun 18 - 2011 Jun 23)
            Might need to look at (1-THETA*log(K[0])), set it to zero if it goes positive.
            ...
            With new large-scale boundary conditions based on probability-flow derivations.
...
/* EXT FILES: none */
/* COMPILE NOTES:
   * To run, type "g++ -lm 3Dvlt_quench.c" without the quotes.
...
const double lmax     = 1.0;      //  1.0  //  maximum of the length scale l (potentially approximate)
const int    lsteps   = 1000;      // 12500, 5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
const double FPconst  = 1.0;       //  Fokker-Planck equation constant arising from Lambda = lambda0*a0*FPconst*(a/a0)^(1/(1-THETA))
const double PflowTOL = 1.0e-5;    //  Probability flow tolerance (program quits with an error message if prob_out-prob_dec > ProbFlowTOL)

const double R        = 8.31447215; // Gas constant
const double DK0      = 5e-7;
//const double Kthresh  = 5.0;

// Constants for fpGRecRel and probability-flow //
...
const double fp5 = THETA/(1.0-THETA)-2.0;                        // = -0.50  (for probability-flow)
...
//			outfile3b = fopen(filename3b,"rw");  // E.g., "3Dvlt_quench_resumeB.out"
			...
			fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, t0, tmax=%g, dt=%g, (dtfrac=%g), a0=%g, a02=%g, a03=%g, a06=%g, FPconst=%g, PflowTOL=%g, fp1=%g, fp2=%g, fp3=%g, fp4=%g, fp5=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,t0,tmax,dt,dtfrac,a0,a02,a03,a06,FPconst,PflowTOL,fp1,fp2,fp3,fp4,fp5);
			fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, t0, tmax=%g, dt=%g, (dtfrac=%g), a0=%g, a02=%g, a03=%g, a06=%g, FPconst=%g, PflowTOL=%g, fp1=%g, fp2=%g, fp3=%g, fp4=%g, fp5=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,t0,tmax,dt,dtfrac,a0,a02,a03,a06,FPconst,PflowTOL,fp1,fp2,fp3,fp4,fp5);
			...
			fprintf(outfile2,"%ld\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", n_t, t, iTfrac, 1.0-iTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, Kr[lpts-1]/K0, G[lpts-1], loopdens);
			fflush(outfile2);
			...
			//while(n_t<=5){
				...
				// no probability-flow beyond lmax is used to derive:
				//    G[lpts-1] = G[lpts-2] + (-PISQ*G[lpts-2]*K[lpts-2]*(1.0-THETA*log(K[lpts-2]))-fp5*G[lpts-2])*dl;
				// or G[lpts-1] = G[lpts-2]/(1.0+PISQ*K[lpts-1]*(1.0-THETA*log(K[lpts-1]))*dl+fp5*dl);
				if( log(K[lpts-1]) > 0.0 ) // if ac' > a, set ac' = a (replace log(K[lpts-1]) with zero)
					G[lpts-1] = G[lpts-2]/(1.0+PISQ*K[lpts-1]*dl+fp5*dl);
				else    G[lpts-1] = G[lpts-2]/(1.0+PISQ*K[lpts-1]*(1.0-THETA*log(K[lpts-1]))*dl+fp5*dl);
				...
				switch(BC){
					...
					case 3: // My bndry cond. Total probability outflow must occur at smallest scale.
						G[0] = (PI*t0*(loopdens_old-loopdens)*dl/(16.0*FPconst*a03) - G[1]*dt) / ( (PISQ*K[0]*(1.0-THETA*log(K[0]))+fp5)*dl*dt + PISQ*t0*dl*dl/(8.0*FPconst) - dt);
						if(G[0]<=G0final)  G[0] = G0final;
						break;
				}
				loopintegral += 0.5*G[0]*exp(3.0*l[0])*dl;
				...
				prob_out = 4.0*FPconst*(a0/t0)*( G[0]*K[0]*(1.0-THETA*log(K[0])) + (fp5*G[0]+(G[1]-G[0])/dl)/PISQ )*4.0*PI*a02;
				prob_dec = -(loopdens-loopdens_old)/dt;
				//printf("\n\nprob_out = %21.21g\tprob_dec = %21.21g", prob_out, prob_dec);
				if(prob_out-prob_dec > PflowTOL){
				...
				if(t>ttrig){
					fprintf(outfile2,"%ld\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", n_t, t, qTfrac, 1.0-qTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, Kr[lpts-1]/K0, G[lpts-1], loopdens);
					fflush(outfile2);
