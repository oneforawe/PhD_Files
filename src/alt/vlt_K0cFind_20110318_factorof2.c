...
/* VERSION: 6 (2011 Mar 17 - 2011 Mar 18)
            C is now out of the recursion relations, and B'=2B (constant increased by factor of 2) */
...
const double B     = 2.0*4.0*PICU/3.0;         // B increased by a factor of 2 !!
...
main(){
	...
	infile   = fopen("vlt_CcK01Input11.dat","r");
	outfile1 = fopen("vlt_CcK0cOutput11.dat","w");
	...
	while(!feof(infile)){
		...
		while(DK0>uncertainty){
			...
			while(1==1){
				...
				estimated = rk4(IN vltRecRel, INOUT estimated, IN 0.0001, IN N);  // Dl=0.0001, n=N=2
