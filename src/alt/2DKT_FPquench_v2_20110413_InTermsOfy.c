...
/* VERSION: 2 (2011 Apr 06 - 2011 Apr 13)
            Gamma should no longer fall below its new equilibrium curve
            the equations are in terms of y when they could be in terms of Gamma (as in version 3, the next version) */
...
const double qTfrac   = 0.85;         //  quench temperature fraction Tq/Tkt
...
main(){
	...
	G0final = exp(-4.0*l[0])*y0*y0/a04;  // = exp(-PISQ*K0)/a04;
	...
	ttrig = 0.0;  // for selecting data to print to outfile2
	while(t<=tmax+dt){
		n++;
		t += dt;
		vpintegral = 0.0;
