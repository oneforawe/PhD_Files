...
/* VERSION: 10 (2012 Jul 19 - 2012 Jul 24)
...
const int    N     = 2;
const double dl          = 1.0e-6; // 1.0e-4
const double ythresh     = 0.5;
...
main(){
	...
	infile   = fopen("3Dvlt_K0cFind_Input38.dat","r");
	//asprintf(&outfilename1, "3Dvlt_K0cFind_Output37.dat");
	asprintf(&outfilename1, "3Dvlt_K0cFind_Output38_uncert%g.dat", uncertainty);
	...
	fprintf(outfile1,"# Parameter values: B=%21.21g, THETA=%g, N=%i, dl=%g, Kthresh=%g, uncertainty=%g, DK01=%g\n", B, THETA, N, dl, Kthresh, uncertainty, DK01);
	// fprintf(outfile,"%s\t%s\n","Cc","K0c");
	printf(         "%s\t%s\n","Cc","K0c");
	fflush(outfile1);
	...
	while(!feof(infile)){
		...
		while(DK0>uncertainty){
			...
			if(K>Kthresh){ // then K0 was too high
				K0 = K0-DK0;
			}
			if(y>ythresh){ // then K0 was too low
				K0 = K0+DK0;
			}
