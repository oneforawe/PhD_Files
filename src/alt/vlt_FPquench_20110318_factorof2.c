...
/* VERSION: 2 (2011 Mar 17 - 2011 Mar 18)
            C is now out of the recursion relations, updated fp constants, and B'=2B (constant increased by factor of 2) */
...
const double B     = 2.0*4.0*PICU/3.0;         // constant increased by factor of 2 !!
...
main(){
	...
	//		case 1:
			P =  0.050;	Cc = 1.1003376541326;	K0c = 0.338730088317275;
...
//		outfile3b = fopen(filename3b,"rw");  // E.g., "vlt_FPquench_resumeB.out"
		...
		fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, tmax=%g, dt0=%g, a0=%g, a06=%g, CONST=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,tmax,dt0,a0,a06,THETA,CONST);
		fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, tmax=%g, dt0=%g, a0=%g, a06=%g, CONST=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,tmax,dt0,a0,a06,THETA,CONST);
...
void rk4(void (*f)(double, double*, double*, unsigned), double *x, double y[], double h, unsigned n){
	...
	for(i=1;i<4;i++){
		...
		f(*x+h*((i+1)/2)/2,s,k[i],n);  // Should this be  f((*x)+h*((i+1)/2)/2,s,k[i],n) ???
	}
...
void fpGRecRel(double x, double Dl, double Gcalc[lpts+1]){
	dGdt[0] = 4*CONST*exp(fp1*x)*(  ( fp2*Gcalc[i] + fp3*(Gcalc[i+1]-Gcalc[i-1])/(2*Dl) + (Gcalc[i+1]-2*Gcalc[i]+Gcalc[i-1])/(Dl*Dl) )/PISQ  +  (1-THETA*log(K[i]))*( fp4*Gcalc[i]*K[i] + (K[i+1]*Gcalc[i+1]-K[i-1]*Gcalc[i-1])/(2*Dl) )  -  THETA*Gcalc[i]*(K[i+1]-K[i-1])/(2*Dl)  );
}
