...
main(){
	...
	asprintf(&filename1, "2DKT_FPquench_vsl_T_%g_%g_lmax%g_dl%g_tmax%g_dt0%g_BC%i_dbug.dat", iTfrac, qTfrac, lmax, dl, tmax, dt0, BC);
	asprintf(&filename2, "2DKT_FPquench_vst_T_%g_%g_lmax%g_dl%g_tmax%g_dt0%g_BC%i_dbug.dat", iTfrac, qTfrac, lmax, dl, tmax, dt0, BC);
	...
	while(n_t<=3){
		...
		for(i=1; i<lpts-1; i++){
			if(i==1) fpGRecRelprint(l[i], dl, G);
			else     fpGRecRel(l[i], dl, G);
			k1[i]     = dGdt[0]*dt;
		...
		for(i=1; i<lpts-1; i++){
			if(i==1) fpGRecRelprint(l[i], dl, testG1);
			else     fpGRecRel(l[i], dl, testG1);
			k2[i]     = dGdt[0]*dt;
		...
		for(i=1; i<lpts-1; i++){
			if(i==1) fpGRecRelprint(l[i], dl, testG2);
			else     fpGRecRel(l[i], dl, testG2);
			k3[i] = dGdt[0]*dt;
