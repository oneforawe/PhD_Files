...
/* DESCRIPTION:
   ...
   * The program uses Gary A. Williams's vortex-loop theory of the superfluid phase transition (i.e., its differential recursion relations) and a 4th-order Runge-Kutta integration method to calculate (length-scale-dependent) thermodynamic quantities (essentially K0,K,Kr,y,e) of the system at a set of temperatures {Tn}, at various length scales (l, no greater than a length scale corresponding to the choice of Dexp), and at some pressure (corresponding to the choice of Cc, the critical dimensionless vortex-loop-core energy).
   * For a set of temperatures {Tn}, the program starts from some initial state (l=0,K0=K0(Cc,Tn),K=K0,Kr=K0,y=y(K0,Cc),e=0) (depending on each temperature Tn and the choice of Cc) at a smallest length scale (l=0 and a=a0), it integrates to larger length scales (incrementing by Dl using the Runge-Kutta technique) up to some limiting length scale (see while loop, where l<Dexp and K<5), and it estimates and records the state at that final length scale (l=lf,K0=K0(Cc,Tn),K=Kf,Kr=Kr(K,l),y=yf,e=ef).
   ...
   * K0c     -
   * dl      -
   * Dexp    - Choosing 100 corresponds to a system with diameter no greater than D = a0*exp(Dexp) ~ 6.7e33 meters (given that a0 ~ 2.5 angstroms at low pressures), allowing for a huge system and calculations at enormous length scales.
...
/* COMPILE NOTES:
   * To run, type "gcc -lm vlt_ThermStates.c vlt_derk.c" without the quotes.
   * NOTE: Be sure that N=3 in vlt_derk.c when compiling (or that the values of N in each file match).
   * NOTE: Update Cc, K0c, and the output filename before running. 
*/
...
#define Kthresh 5.0
#define Cc      1.20
#define K0c     0.2805443089752288
#define dl      0.0001
#define Dexp    100  // 100 corresponds to largest length scale l_max = D = a0*exp(Dexp) ~ 6.8e33 meters (huge diameter system)
...
main(){
	...
	outfile = fopen("vlt_ThermStates_Dexp100_A_Cc1.20.dat","w");  //  E.g., "vlt_ThermStates.out" or "vlt_ThermStates_Dexp100_A_Cc0.00.dat"
	...
	for(i=0; i<60; i++){
		...
		while(estimated.l<Dexp && estimated.thrmv[0]<Kthresh){
		...
		/* Update values that depend on evolved state-variables, if not using "Print out intermediate numbers" block of code */
		Kr = estimated.thrmv[0]*exp(-estimated.l);
