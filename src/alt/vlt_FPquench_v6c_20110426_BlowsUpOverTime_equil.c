...
/* VERSION: 6 (2011 Apr 12 - 2011 Apr 26)
            In this version I tried (but gave up) to set lsteps to a larger integer type and make the program work (so that the equilibrium superfluid fraction curve keeps falling logrithmically downward as it should.  (see folder 3D_08/equil_better)

            Not allowing ac' to exceed a, not allowing Gamma to fall below its equilibrium curve.
            Also, I simply deleted references to Kthresh in this version.  Later, if this version works nicely, I could revert to the previous form of code that doesn't use newlpts. */
...
const double lmax     = 10.0;      //10.0; 1.0  //  maximum of the length scale l (potentially approximate)
const int64_t lsteps   = 1.25e6;      //  5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
const int64_t lpts     = lsteps+1;  //  from l=0 to l=lmax, inclusive (incl. the boundary-condition-enforcing pnt, there are lpts+1 "l" pnts)
...
main(){
	...
	//	}
		...
		for(s2=1; s2<=5; s2++){
			...
			asprintf(&filename1, "vlt_FPquench_v6c_vsl_3_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt0%g_equil.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt0);
			asprintf(&filename2, "vlt_FPquench_v6c_vst_3_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt0%g_equil.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt0);
...
//			outfile3b = fopen(filename3b,"rw");  // E.g., "vlt_FPquench_resumeB.out"
			...
			fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%ld, (dl=%g), lpts=%ld, ldatamax=%i, tmax=%g, dt0=%g, a0=%g, a03=%g, a06=%g, FPconst=%g, Lconst=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,tmax,dt0,a0,a03,a06,FPconst,Lconst);
			fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%ld, (dl=%g), lpts=%ld, ldatamax=%i, tmax=%g, dt0=%g, a0=%g, a03=%g, a06=%g, FPconst=%g, Lconst=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,tmax,dt0,a0,a03,a06,FPconst,Lconst);
...
//				dt = dt0 + Dt*(1-exp(-t));  // added
				...
				if( THETA*log(K[newlpts-1]) > 0.0 ) // if ac' > a, set ac' = a (set THETA*log(K[newlpts-1]) to zero)
					G[newlpts-1] = G[newlpts-2]/(1.0+PISQ*K[newlpts-1]*dl);                                //*
				else
					G[newlpts-1] = G[newlpts-2]/(1.0+PISQ*K[newlpts-1]*(1.0-THETA*log(K[newlpts-1]))*dl);  //*
...
//				K[newlpts] = K[newlpts-1];
				...
				if( (t>=1e-2&&j==1) || (t>=2e-2&&j==2) || (t>=3e-2&&j==3) || (t>=4e-2&&j==4) || (t>=5e-2&&j==5) || (t>=6e-2&&j==6) || (t>=7e-2&&j==7) || (t>=8e-2&&j==8) || (t>=9e-2&&j==9)  || (t>=1e-1&&j==10) ){
				  //(t>=1e0&&j==3) || (t>=1e1&&j==4) || (t>=2e1&&j==5) || (t>=3e1&&j==6) || (t>=4e1&&j==7) || (t>=5e1&&j==8) || (t>=1e2&&j==9) ){  // 1==0    ){// 
					fprintf(outfile1,"\n# time step n_t = %ld\tt = %e\t(dt = %e)\n", n_t,t,dt);
					fprintf(outfile1,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","Kr/K0","G");
					for(i=0; i<newlpts; i++){
