...
/* FILENAME: 2DKT_FPquench_inject_SteadyState.c */
/* VERSION: 1 (2011 May 11 - 2011 May 23)
...
const double iTfrac   = 1.0;        //  initial temperature fraction Ti/Tkt
...
const double InjConst = 1.0;          //  injection constant - the factor multiplying the Heaviside step function, gamma*t0/(2*PI*a02), where gamma is the factor multiplying the dirac delta source
const double dGdl0    = -1e-3;        //  initial Gamma (G) derivative, dG/dl at l=0
...
main(){
	...
	asprintf(&filename1, "2DKT_FPquench_inject_SteadyState_vsl_T_%g_lmax%g_dl%g_linj%g_InjConst%g_dGdl0%g.dat", iTfrac, lmax, dl, linj, InjConst, dGdl0);
	asprintf(&filename2, "2DKT_FPquench_inject_SteadyState_vst_T_%g_lmax%g_dl%g_linj%g_InjConst%g_dGdl0%g.dat", iTfrac, lmax, dl, linj, InjConst, dGdl0);
	...
	fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, linj=%g, InjConst=%g, dGdl0=%g, tmax=%g, dt0=%g, a0=%g, a04=%g, K0c=%g\n", iTfrac,qTfrac,lmax,lsteps,dl,lpts,ldatamax,linj,InjConst,dGdl0,tmax,dt0,a0,a04,K0c);
	fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, linj=%g, InjConst=%g, dGdl0=%g, tmax=%g, dt0=%g, a0=%g, a04=%g, K0c=%g\n", iTfrac,qTfrac,lmax,lsteps,dl,lpts,ldatamax,linj,InjConst,dGdl0,tmax,dt0,a0,a04,K0c);
...
void SteadyStateRecRel(double x, double z[2], double dzdx[2], unsigned n){
	...
	if(x>linj)
		dzdx[1] = 2.0*PI*K[0]*G[0] + dGdl0 - 2.0*PI*z[0]*z[1] - InjConst*exp(2.0*linj);
	else
		dzdx[1] = 2.0*PI*K[0]*G[0] + dGdl0 - 2.0*PI*z[0]*z[1];
}
