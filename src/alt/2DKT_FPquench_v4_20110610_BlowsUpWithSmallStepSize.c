...
/* VERSION: 4 (2011 May 26 - 2011 Jun 10)
            Next version could get rid of unnecessary boundary conditions, including extra point (lpts+1).
...
const int    BC       = 1;            //  Boundary Condition choice 1=Hanching's, 2=Gary's, 3=Mine
...
main(){
	...
	asprintf(&filename1, "2DKT_FPquench_vsl_T_%g_%g_lmax%g_dl%g_tmax%g_dt0%g_BC%i.dat", iTfrac, qTfrac, lmax, dl, tmax, dt0, BC);
	asprintf(&filename2, "2DKT_FPquench_vst_T_%g_%g_lmax%g_dl%g_tmax%g_dt0%g_BC%i.dat", iTfrac, qTfrac, lmax, dl, tmax, dt0, BC);
