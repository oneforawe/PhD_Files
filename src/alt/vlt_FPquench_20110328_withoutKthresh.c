...
// This program is buggy since it allows calculations to continue after K has diverged (see vlt_K0cFind.c, and use of Kthresh).

/* FILENAME: vlt_FPquench.c */
/* VERSION: 3 (2011 Mar 18 - 2011 Mar 28)
...
const int    lsteps   = 2000;      //  1250, 5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
main(){
	...
	//	}
		...
		for(s2=1; s2<=7; s2++){
			switch(s2){
				case 1: iTfrac = 1.00; break;
				case 2: iTfrac = 0.98; break;
				case 3: iTfrac = 0.95; break;
				case 4: iTfrac = 0.75; break;
				case 5: iTfrac = 0.50; break;
			}

			// Prepare output files, print identification and values //
			asprintf(&filename1, "vlt_FPquench_vsl_3_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt0%g_equil.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt0);
			asprintf(&filename2, "vlt_FPquench_vst_3_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt0%g_equil.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt0);
...
//			outfile3b = fopen(filename3b,"rw");  // E.g., "vlt_FPquench_resumeB.out"
			...
			fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, tmax=%g, dt0=%g, a0=%g, a03=%g, a06=%g, CONST=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,tmax,dt0,a0,a03,a06,THETA,CONST);
			fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, tmax=%g, dt0=%g, a0=%g, a03=%g, a06=%g, CONST=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,tmax,dt0,a0,a03,a06,THETA,CONST);
...
//				dt = dt0 + Dt*(1-exp(-t));  // added
				...
				if( 1==0    ){//  (t>=1e-2&&j==1) || (t>=1e-1&&j==2) || (t>=1e0&&j==3) || (t>=2e0&&j==4) || (t>=5e0&&j==5) || (t>=1e1&&j==6) || (t>=2e1&&j==7) || (t>=3e1&&j==8) || (t>=4e1&&j==9) || (t>=5e1&&j==10) ){
					fprintf(outfile1,"\n# time step n = %ld\tt = %e\t(dt = %e)\n", n,t,dt);
					fprintf(outfile1,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","Kr/K0","G");
					for(i=0; i<lpts; i++){
