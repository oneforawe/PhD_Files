...
// forget fiddling with RK, see constraints below in NOTE and "Program parameters/inputs"
...
/* COMPILE NOTES:
   ...
   * NOTE: Be sure lsteps and dt0 meet constraints listed below in "Program parameters/inputs".
*/
...
const int    lsteps   = 2200;         //  1250, 5000, 100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
main(){
	...
	asprintf(&filename1, "2DKT_FPquench_vsl_T_%g_%g_lmax%g_dl%g_tmax%g_dt0%g_BC%i_dbug_NewRK2v2.dat", iTfrac, qTfrac, lmax, dl, tmax, dt0, BC);
	asprintf(&filename2, "2DKT_FPquench_vst_T_%g_%g_lmax%g_dl%g_tmax%g_dt0%g_BC%i_dbug_NewRK2v2.dat", iTfrac, qTfrac, lmax, dl, tmax, dt0, BC);
...
void fpGRecRelmidstep(double x, double Dl, double Gcalc[lpts]){
        take mid-steps, using version two
	dGdt[0] = exp(-2*x)*( PI*(K[i+1]*Gcalc[i+1]-K[i-1]*Gcalc[i-1])/Dl + (Gcalc[i+1]-2*Gcalc[i]+Gcalc[i-1])/(Dl*Dl) );
}
*/
