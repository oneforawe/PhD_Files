...
const int     lmax     = 10;        //  1
const int     lsteps   = 1250;      //  100000 //  lsteps = lmax/dl = 10/0.0001 = 100,000  (make sure this is divisible by ldatamax)
...
const int64_t tsteps   = 7e11;      //  (later, 10000) max unitless time (see if(t==...) below)
const double  dt       = 1.80161903338e-4;  // 3e-8 //  the time increment (in units of the "diffusion time")
const double  a0       = 1.0;
...
/*		testKG[0][2].c[1]      = testKG[1][2].c[1];
		...
		if( (t>0.4&&k==1) || (t>2&&k==2) || (t>16&&k==3) || (t>206&&k==4) || (t>2060&&k==5) || (t>20600&&k==6) ){  //  (log10(t)%1==0){
			fprintf(outfile,"\n# time step n = %ld\tt = %e\t(dt = %e)\n\n", n,t,dt);
			fprintf(outfile,"%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","G");
			for(i=0; i<lpts; i++){
				/* Print at most [ldatamax] (e.g., 1000) data points to screen and output file */
