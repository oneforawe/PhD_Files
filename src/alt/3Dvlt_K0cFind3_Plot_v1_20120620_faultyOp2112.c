...
const int    OpC  = 2;   //  Option C   Set (2nd and later steps) direction and const dl steps or limited ds steps (lengths in l-space or K-y-space)
...
const double uncertainty     = 1.0e-8;  // could loop, decreasing dK until within K0c is uncertainty
...
int main(){
	...
	if( OpB!=1 ){
		...
		fprintf(outfile2,"# Initialized values: Deltas=%g, Deltal=%g, count_steps=%i, count_loops=%i, K0c=%g, oldK0c=%g\n", Deltas, Deltal, count_steps, count_loops, K0c, oldK0c);
		...
		fprintf(outfile2,"%i\t%17.17g\t%17.17g\t%g\t%g\n",count_steps,K,y,Deltal,Deltas);
		printf(          "%i\t%17.17g\t%17.17g\t%g\t%g\n",count_steps,K,y,Deltal,Deltas);
		fflush(outfile2);
	...
	if( OpA==2 && OpAa==1 && OpB==1 && (OpC==2||OpC==4) ){
		...
		infile  = fopen(        "3Dvlt_K0cFind3_Plot_Input1.dat","r");
		asprintf(&outfilename1, "3Dvlt_K0cFind3_Plot_Output1_OpC%i_uncert%g.dat", OpC,uncertainty);
		...
		fprintf(outfile1,"# Initialized values: Deltas=%g, Deltal=%g, count_steps=%i, count_loops=%i, K0c=%g, oldK0c=%g\n", Deltas, Deltal, count_steps, count_loops, K0c, oldK0c);
		...
		while(!feof(infile)){
			// Re-initialize starting point (for second and later passes of while loop) //
			 K = Kstar;
			 y = ystar;
					...
					if(initialKdir>0){
						while(nextK0cfound==false){
							if( y > exp(-PISQ*Cc*K) ){
								...
								Deltas += sqrt((K-oldK)*(K-oldK)+(y-oldy)*(y-oldy));
								fprintf(outfile2,"%i\t%17.17g\t%17.17g\t%g\t%g\tmade it to the end\n",count_steps,K,y,Deltal,Deltas);
								printf(          "%i\t%17.17g\t%17.17g\t%g\t%g\tmade it to the end\n",count_steps,K,y,Deltal,Deltas);
								fflush(outfile2);
							}
							else{
								oldK = K;
...
//				}
			...
			//fprintf(outfile1,"%17.17g\t%17.17g\n",Cc,K0c);
			fprintf(outfile1,"%17.17g\t%17.17g\t(steps=%i, loops=%i, dK=%g)\n",Cc,K0c,count_steps,count_loops,dK);
			printf(          "%17.17g\t%17.17g\t(steps=%i, loops=%i, dK=%g)\n",Cc,K0c,count_steps,count_loops,dK);
			fflush(outfile1);
