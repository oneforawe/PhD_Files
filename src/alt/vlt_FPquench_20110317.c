...
/* VERSION: 1 (2011 Jan 18 - 2011 Mar 17)
...
const double dt0      = 1.0e-12;    //  the time increment (in units of the "diffusion time")
//const double dtf      = 1.0e-4;    //  the time increment (in units of the "diffusion time")
const double a0       = 1.0;       //  a0 in units of a0 (!)
const double a06      = 1.0;       //  a0 to the sixth power
...
const double fp1 = -3+THETA/(1-THETA);  // = -1.5
const double fp2 = -3*THETA/(1-THETA);  // = -4.5
const double fp3 = -3+THETA/(1-THETA);  // = -1.5
const double fp4 = THETA/(1-THETA);     // =  1.5
...
// fpGRecRel(x,Dl,Gcalc[lpts+1],C);
void rk4(void (*f)(double, double*, double*, unsigned int, double), double *x, double y[], double h, unsigned n, double c);
void EqRecRel(double x, double z[], double dzdx[], unsigned n, double C);
void fpKRecRel(double x, double Kcalc[1], double dKdx[1], unsigned n, double C);
void fpGRecRel(double x, double Dl, double Gcalc[lpts+1], double C);
...
main(){
	...
	//for(s=1; s<=7; s++){
	//	switch(s){ // pressure P in bars (See RhosAmps.ods for Cc(P), derived from Ahlers' 1973 data)
			//case 1:
			P =  0.050;  Cc = 1.105023144261770;  K0c = 0.295611433115442;  //break;
	//		case 2:  P =  1.646;  Cc = 1.058195551375160;  K0c = 0.303870330660806;  break;
	//		case 3:  P =  7.328;  Cc = 0.905924430200288;  K0c = 0.335617836037364;  break;
	//		case 4:  P = 15.031;  Cc = 0.730725772346878;  K0c = 0.385625265795951;  break;
	//		case 5:  P = 18.180;  Cc = 0.667963030000677;  K0c = 0.408889237394402;  break;
	//		case 6:  P = 22.533;  Cc = 0.588565898089815;  K0c = 0.444331744979223;  break;
	//		case 7:  P = 25.868;  Cc = 0.532988906023542;  K0c = 0.474486704164683;  break;
	//	}

		// Prepare output files, print identification and values //
		asprintf(&filename1, "vlt_FPquench_vsl_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt0%g.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt0);
		asprintf(&filename2, "vlt_FPquench_vst_T_%g_%g_P_%06.3f_lmax%g_dl%g_tmax%g_dt0%g.dat", iTfrac, qTfrac, P, lmax, dl, tmax, dt0);
...
//		outfile3b = fopen(filename3b,"rw");  // E.g., "vlt_FPquench_resumeB.out"
		...
		fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, tmax=%g, dt0=%g, a0=%g, a06=%g, THETA=%g, CONST=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,tmax,dt0,a0,a06,THETA,CONST);
		fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, P=%g (Cc=%g, K0c=%g), lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, tmax=%g, dt0=%g, a0=%g, a06=%g, THETA=%g, CONST=%g\n", iTfrac,qTfrac,P,Cc,K0c,lmax,lsteps,dl,lpts,ldatamax,tmax,dt0,a0,a06,THETA,CONST);
		...
		fprintf(outfile2,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "time-step","t","T/Tc","1-T/Tc","l","K","K/K0","Kr/K0","G","vldens");
		...
		for(i=1; i<lpts; i++){
			...
			rk4(EqRecRel, &x, z, dl, 2, Cc);  // equil: rk4 2D (K and y), EqRecRel
			l[i] = l[i-1] + dl;
			K[i] = z[0];
			Kr[i] = K[i]*exp(-l[i]);
			G[i] = z[1]*exp(-6*l[i])/a06;
			vlintegral += G[i]*exp(3*l[i])*dl;
		...
		vldens = 4*PI*a0*a0*a0*vlintegral;
		fflush(outfile1);
		...
		fprintf(outfile2,"%ld\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", n, t, iTfrac, 1-iTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, Kr[lpts-1]/K0, G[lpts-1], vldens);
		fflush(outfile2);
		...
		Kcalc[0] = K[0];
		for(i=1; i<lpts; i++){
			rk4(fpKRecRel, &x, Kcalc, dl, 1, Cc);  // nequil: rk4 1D (K), fpKRecRel
			K[i] = Kcalc[0];
		}
...
//			dt = dt0 + Dt*(1-exp(-t));  // added
			...
			vlintegral += G[0]*exp(3*l[0])*dl;
			vlintegral += G[lpts-1]*exp(3*l[lpts-1])*dl;
			vldens = 4*PI*a0*a0*a0*vlintegral;
			...
			if(t>ttrig){
				fprintf(outfile2,"%ld\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", n, t, qTfrac, 1-qTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, Kr[lpts-1]/K0, G[lpts-1], vldens);
				fflush(outfile2);
...
void rk4(void (*f)(double, double*, double*, unsigned, double), double *x, double y[], double h, unsigned n, double c){
	int i,j;
	double *k[4],*s,temp;
	...
	for(i=1;i<4;i++){
		...
		f(*x+h*((i+1)/2)/2,s,k[i],n,c);  // Should this be  f((*x)+h*((i+1)/2)/2,s,k[i],n) ???
	}
...
void EqRecRel(double x, double z[], double dzdx[], unsigned n, double C){
	...
	dzdx[1] = z[1]*( 6.0 - PISQ*z[0]*(1.0+C-THETA*log(z[0])) );
...
void fpGRecRel(double x, double Dl, double Gcalc[lpts+1], double C){
	dGdt[0] = 4*CONST*exp(fp1*x)*(  ( fp2*Gcalc[i] + fp3*(Gcalc[i+1]-Gcalc[i-1])/(2*Dl) + (Gcalc[i+1]-2*Gcalc[i]+Gcalc[i-1])/(Dl*Dl) )/PISQ  +  (1+C-THETA*log(K[i]))*( fp4*Gcalc[i]*K[i] + (K[i+1]*Gcalc[i+1]-K[i-1]*Gcalc[i-1])/(2*Dl) )  -  THETA*Gcalc[i]*(K[i+1]-K[i-1])/(2*Dl)  );
}
...
vortex loop density
vldens = int_a0^amax G*d^3a = int_a0^amax int_0^2PI G*a^2*dOmega*da = 4PI int_a0^amax G*a^2*da = 
a = a0*exp(l)
da = a0*exp(l)*dl
a^2*da = a0^3*exp(3l)*dl

vldens = 4PI*a0^3 int_0^lmax G*exp(3l)*dl
vlintegral += G*exp(3l)*dl
vldens = 4PI*a0^3*vlintegral
