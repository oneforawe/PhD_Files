...
/* VERSION: 7 (2011 Sep 13 - ...)
            Changing correlation length from "xi" to "lc1", as in l_correlation.
            Adding zscale, xi, etc to examine behavior of pairdens (aka rho) over time.
...
/* DESCRIPTION:
   ...
   * The properties calculated are superfluid ratios (K, K/K0=sigma_s/sigma_0) and vortex-pair number density G at various length-scales (l).
*/
/* EXT FILES: none */
/* COMPILE NOTES:
...
const double iTfrac    = 0.99;         //  initial temperature fraction Ti/Tkt
...
const double tmax      = 100;          //  10000.0 //  max time (in units of the diffusion time)
...
int main(){
	...
	double  dGda[lpts], dGdaMin;
	double  Ki[lpts], Gi[lpts], dlnGdli[lpts];
	double  z[2], Kcalc[lpts], Gcalc[lpts];
	double  k1[lpts], k2[lpts], k3[lpts];
	double  testG1[lpts], testG2[lpts];
	double  t, dt, dtfrac, ttrig;
	double  pairintegral, pairdens, pairdens_old; // aka rho
	double  dlnrhodlnt;
	double  lc1, zscale1a, zscale1b, rhopowthry1a, rhopowthry1b;
	double  lc2, zscale2a, zscale2b, rhopowthry2a, rhopowthry2b;
	...
	asprintf(&filename1, "2Dvpt_quench_vsl_T_%g_%g_lmax%g_dl%g_tmax%g_dt%g_BC%i_new.dat", iTfrac, qTfrac, lmax, dl, tmax, dt, BC);
	asprintf(&filename2, "2Dvpt_quench_vst_T_%g_%g_lmax%g_dl%g_tmax%g_dt%g_BC%i_new.dat", iTfrac, qTfrac, lmax, dl, tmax, dt, BC);
	...
	fprintf(outfile1,"# Parameter values: iTfrac=%g, qTfrac=%g, lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, t0=%g, tmax=%g, dt=%g, (dtfrac=%g), a0=%g, a02=%g, a04=%g, K0c=%g, NDflowTOL=%g\n", iTfrac,qTfrac,lmax,lsteps,dl,lpts,ldatamax,t0,tmax,dt,dtfrac,a0,a02,a04,K0c,NDflowTOL);
	fprintf(outfile2,"# Parameter values: iTfrac=%g, qTfrac=%g, lmax=%g, lsteps=%i, (dl=%g), lpts=%i, ldatamax=%i, t0=%g, tmax=%g, dt=%g, (dtfrac=%g), a0=%g, a02=%g, a04=%g, K0c=%g, NDflowTOL=%g\n", iTfrac,qTfrac,lmax,lsteps,dl,lpts,ldatamax,t0,tmax,dt,dtfrac,a0,a02,a04,K0c,NDflowTOL);
	...
	x = l[0] = 0.0;
	z[0] = K[0] = K0 = Ki[0] = K0c/iTfrac;
	z[1] = G[0] = G0 = Gi[0] = exp(-4.0*l[0]-PISQ*K0)/a04;
	pairintegral = 0.0;
	pairintegral += 0.5*G[0]*exp(2.0*l[0])*dl;
	w1  = 0;  // correlation length integer
	w2  = 0;  // correlation length integer
	lc1 = l[w1];
	lc2 = l[w2];

	// Print data labels to screen and output files //
	printf(          "\n# time step n_t = %ld\tt = %e\t(dt = %e)\n", n_t,t,dt);
	fprintf(outfile1,"\n# time step n_t = %ld\tt = %e\t(dt = %e)\n", n_t,t,dt);
	printf(            "%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","G","dGda");
	fprintf(outfile1,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","G","dGda");
	fprintf(outfile2,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "time-step", "t", "T/Tc","1-T/Tc","l","K","K/K0","G","pairdens","d(lnrho)/d(lnt)","lc1","lc2","rhopowthry1a","rhopowthry1b","rhopowthry2a","rhopowthry2b");
	...
	for(i=1; i<lpts; i++){
		...
		K[i] = Ki[i] = z[0];
		G[i] = Gi[i] = z[1];
		dlnGdli[i-1] = (log(Gi[i])-log(Gi[i-1]))/dl;
		dGda[i-1]    = exp(-l[i-1])*(G[i]-G[i-1])/dl/a0;
		pairintegral += G[i]*exp(2.0*l[i])*dl;
		// Print at most [ldatamax] (e.g., 50) data points to screen and output file 1 //
		if((i-1)%(lsteps/ldatamax)==0){
			printf(          "%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1.0-iTfrac, l[i-1], K[i-1], K[i-1]/K0, G[i-1], dGda[i-1]);
			fprintf(outfile1,"%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1.0-iTfrac, l[i-1], K[i-1], K[i-1]/K0, G[i-1], dGda[i-1]);
		}
	}
	printf(          "%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1.0-iTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, G[lpts-1], dGda[lpts-1]);
	fprintf(outfile1,"%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1.0-iTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, G[lpts-1], dGda[lpts-1]);
	fflush(outfile1);
	pairdens = 2.0*PI*a02*pairintegral;
	dlnrhodlnt = 0.0;
	zscale1a = 2.0*PI*Ki[w1] - 2.0;
	zscale2a = 2.0*PI*Ki[w2] - 2.0;
	zscale1b = dlnGdli[w1] - 2.0;
	zscale2b = dlnGdli[w2] - 2.0;
	rhopowthry1a = -zscale1a/2.0;
	rhopowthry1b = -zscale1b/2.0;
	rhopowthry2a = -zscale2a/2.0;
	rhopowthry2b = -zscale2b/2.0;

	// Print out the data at largest length scale to file 2 //
	fprintf(outfile2,"%ld\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", n_t, t, iTfrac, 1.0-iTfrac, l[lpts-1], K[lpts-1], K[lpts-1]/K0, G[lpts-1], pairdens, dlnrhodlnt, lc1, lc2, rhopowthry1a, rhopowthry1b, rhopowthry2a, rhopowthry2b);
	fflush(outfile2);
	...
	// Advance in time //
	j = 0;        // for selecting times to print out l-dependent data: outfile1
	...
	//while(n_t<=5){
		...
		// no number-density-flow beyond lmax is equivalent to the equilibrium relation, dG/dl = -2piKG, which is used to derive:
		...
		numden_out = (a0/t0)*(2.0*PI*G[0]*K[0]+(G[1]-G[0])/dl)*2.0*PI*a0;
		numden_dec = -(pairdens-pairdens_old)/dt;
		//printf("\n\nnumden_out = %21.21g\tnumden_dec = %21.21g", numden_out, numden_dec);
		if(numden_out-numden_dec > NDflowTOL){
			printf("\n\nnumden_out - numden_dec = %g", numden_out-numden_dec);
			printf("\nCode Problem: Vortex-pair number-density outflow doesn't equal decrease in number-density!");
			printf("\n(This probably means that the time-step size dt is too large.)\n");
			exit(EXIT_FAILURE);
		}

		// Find (versions 1&2 of) quench correlation length, zscale, and "local exponents" rhopowthry and dlnrhodlnt //
		for(i=w1+1; i<w1+10; i++){
			if(Gi[i]/G[i] > 1.25)  w1 = i;
		}
		lc1 = l[w1];
		zscale1a = 2.0*PI*Ki[w1] - 2.0;
		zscale1b = dlnGdli[w1] - 2.0;
		...
		rhopowthry1b = -zscale1b/2.0;
		for(i=0; i<lpts-1; i++){
			dGda[i] = exp(-l[i])*(G[i+1]-G[i])/dl/a0;
			if(dGda[i] < dGdaMin){  dGdaMin=dGda[i];  w2 = i;}
		}
		lc2 = l[w2];
		zscale2a = 2.0*PI*Ki[w2] - 2.0;
		zscale2b = dlnGdli[w2] - 2.0;
		rhopowthry2a = -zscale2a/2.0;
		rhopowthry2b = -zscale2b/2.0;
		dlnrhodlnt = log(pairdens/pairdens_old)/log(t/(t-dt));
		...
		if(   j==0            || (t>=1e-4&&j==1) || (t>=1e-3&&j==2) || (t>=1e-2&&j==3)
		   || (t>=1e-1&&j==4) || (t>=2e-1&&j==5) || (t>=3e-1&&j==6) || (t>=4e-1&&j==7) || (t>=5e-1&&j==8) || (t>=6e-1&&j==9) || (t>=7e-1&&j==10) || (t>=8e-1&&j==11) || (t>=9e-1&&j==12)
		   || (t>=1e0&&j==13) || (t>=2e0&&j==14) || (t>=3e0&&j==15) || (t>=4e0&&j==16) || (t>=5e0&&j==17) || (t>=6e0&&j==18) || (t>=7e0&&j==19) || (t>=8e0&&j==20) || (t>=9e0&&j==21)
		   || (t>=1e1&&j==22) || (t>=2e1&&j==23) || (t>=3e1&&j==24) || (t>=4e1&&j==25) || (t>=5e1&&j==26) || (t>=6e1&&j==27) || (t>=7e1&&j==28) || (t>=8e1&&j==29) || (t>=9e1&&j==30) || (t>=1e2&&j==31) ){
			fprintf(outfile1,"\n# time step n_t = %ld\tt = %e\t(dt = %e)\n", n_t,t,dt);
			fprintf(outfile1,"# %s\t%s\t%s\t%s\t%s\t%s\t%s\n", "T/Tc","1-T/Tc","l","K","K/K0","G","dGda");
			for(i=0; i<lpts; i++){
				// Print at most [ldatamax] (e.g., 1000) data points to screen and output file //
				if(i%(lsteps/ldatamax)==0){
					printf(          "%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1.0-iTfrac, l[i], K[i], K[i]/K0, G[i], dGda[i]);
					fprintf(outfile1,"%g\t%g\t%g\t%g\t%g\t%g\t%g\n", iTfrac, 1.0-iTfrac, l[i], K[i], K[i]/K0, G[i], dGda[i]);
				}
			}
			fflush(outfile1);
		...
		if(t>ttrig){
			...
			k++;
			// Sparse update //
			printf(          "n_t=%ld  t=%e  (dt=%e)  l=%g  K/K0=%g  G=%g  pairdens=%g  (Tfrac: %g->%g)  lc1=%g  (exp: calc=%g thry=%g)\n", n_t, t, dt, l[lpts-1], K[lpts-1]/K0, G[lpts-1], pairdens, iTfrac, qTfrac, lc1, dlnrhodlnt, rhopowthry1a);
		}
